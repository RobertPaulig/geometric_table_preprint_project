...
h=200,\qquad K=200,
\]
and two weight modes: $w\equiv 1$ and the IDF weight.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\linewidth]{fig/core_gap_scatter_edges_ones.png}
...
of core GC gap on twin label, core edges, core GC fraction, and $\log K_{\text{used}}$ yields $\beta_{\text{twin}}\approx -0.017$ with permutation
...
($\text{twin\_isolates}$) $\beta_{\text{twin}}\approx 0.408$ (perm-$p\approx 2\times 10^{-4}$),
and twin degree sum ($\text{twin\_deg\_sum}$) $\beta_{\text{twin}}\approx -0.878$ (perm-$p\approx 2\times 10^{-4}$).
...
\begin{verbatim}
PYTHONPATH=code python code/scripts/batch_centers.py --center-min 500 --center-max 200000 \
...
PYTHONPATH=code python code/scripts/analyze_batch.py --input out/batch_summary_six_ones_core_autok.csv \
  --out-dir out --label six_ones_core_autok --permutation-iters 5000 --bootstrap-iters 1000
PYTHONPATH=code python code/scripts/matched_twins.py --center-min 500 --center-max 200000 \
