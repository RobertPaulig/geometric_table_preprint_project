...
We simulate a post-TF regime by conditioning on candidates that survive up to a TF-dependent
limit $Q_{\mathrm{TF}}$ and predicting which of those will die later by a deeper horizon
$Q_{\mathrm{horizon}}$. The goal is not to reject cheap trash, but to prioritize additional
factoring to avoid expensive PRP/LL on doomed survivors.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{../out/wave_atlas/m28_gimps/m28_auc_by_tf_bits.png}
\caption{AUC on the conditional task (dies by horizon among passes\_tf=1) versus TF depth.}
\end{figure}
...
\caption{Post-TF metrics by TF depth (M28).}
\end{table}
